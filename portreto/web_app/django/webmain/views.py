...
def detail(request, gallery_id, username=None, token=None):
    ...
    reactions = api_client.get_gallery_reaction(requsername=requsername, galleryid=gallery_id)
...
def photo_detail(request, gallery_id, photo_id, username=None, token=None):
    ...
    reactions = api_client.get_photo_reaction(requsername=requsername,photoid = photo_id)
...
def like_photo(request, gallery_id, photo_id, username=None, token=None):
    ...
    return redirect('webmain:photo_detail', gallery_id, photo_id)   #TODO CHECK REDIRECTION
...
def search(request, username=None, token=None):
    ...
    if query == "":
        ...
        for profile in profiles:
            profile.is_friend = len(api_client.get_follow(FC_1_Username=username,FC_2_Username=profile.user.username))>0
        context={
            'profiles': profiles,
        }
